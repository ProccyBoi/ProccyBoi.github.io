(() => {
  ...
  // Source coordinates store KiCad's downward Y as negative Z. Reflect the
  // complete assembly into Three's +Y-up frame so neither face is mirrored;
  // artwork, holes and components keep their shared source registration.
  board.scale.z = -1;
  ...
  const setPreset = (name) => {
    if (name === 'top') { targetYaw=d2r(config.boardRotation || 0); targetPitch=d2r(89.2); targetDistance=fitDistance(true); }
    else if (name === 'bottom') { targetYaw=d2r((config.boardRotation || 0) + 180); targetPitch=d2r(-89.2); targetDistance=fitDistance(true); }
