(() => {
  ...
  if (config.ledInstances?.length) {
    // TramTrace carries 116 2 × 2 mm LEDs. Instancing keeps all source
    // placements/rotations while reducing draw calls enough for smooth orbiting.
    const g = new THREE.Group(); board.add(g);
    const ledW=config.ledWidth||2.2, ledD=config.ledDepth||2.0, ledH=config.ledHeight||0.84, count=config.ledInstances.length;
    const bodyGeo=new THREE.BoxGeometry(ledW,ledH,ledD);
    const winGeo=new THREE.BoxGeometry(ledW*0.56,0.035,ledD*0.56);
    const padGeo=new THREE.BoxGeometry(ledW*0.28,0.035,ledD*0.34);
    const notchGeo=new THREE.BoxGeometry(Math.min(ledW,ledD)*0.13,0.028,Math.min(ledW,ledD)*0.13);
    ...
    config.ledInstances.forEach((p,i)=>{
      const rot=d2r(p[2]||0);
      dummy.position.set(p[0],config.thickness/2+ledH/2,p[1]); dummy.rotation.set(0,rot,0); dummy.scale.set(1,1,1); dummy.updateMatrix(); bodies.setMatrixAt(i,dummy.matrix);
      dummy.position.y=config.thickness/2+ledH+0.045; dummy.updateMatrix(); windows.setMatrixAt(i,dummy.matrix);
      const c=Math.cos(rot),ss=Math.sin(rot),offX=ledW*0.47,offZ=ledD*0.47;
      [[-offX,-offZ],[offX,-offZ],[-offX,offZ],[offX,offZ]].forEach((q,k)=>{
      ...
      const nx=-ledW*0.32,nz=-ledD*0.32;
      dummy.position.set(p[0]+nx*c+nz*ss,config.thickness/2+ledH+0.07,p[1]-nx*ss+nz*c);dummy.rotation.set(0,rot,0);dummy.updateMatrix();notches.setMatrixAt(i,dummy.matrix);
    });
  ...
  // procedural scene above remains as a resilient fallback for browsers that
  ...
  if (config.exactModel && THREE.GLTFLoader) {
    ...
    loader.load(config.exactModel, (gltf) => {
      ...
      refs.forEach((ref, index) => {
        ...
        if (!node) return;
        const spec = componentByRef.get(ref) || ledByRef.get(ref);
        node.userData.ref = ref;
        node.userData.name = spec.name || ref;
        node.userData.copy = spec.copy || '';
        pickables.push(node);
